...
public static class CameraUtility
{
    ...
    {
        var positionByCamera = position.GetPositionInWorldToViewportPointCamera();
        return positionByCamera.x > valuePositive || positionByCamera.y > valuePositive || positionByCamera.z < 0;
    }
    public static bool IsOutCameraNegative(this Vector3 position, float valueNegative)
    {
        var positionByCamera = position.GetPositionInWorldToViewportPointCamera();
        return positionByCamera.x < valueNegative || positionByCamera.y < valueNegative || positionByCamera.z < 0;
    }
}
